...
`mmkc-creator-image2psd-skill` 是一个 Codex skill，用来把一张或多张栅格图片整理成可编辑的分层 PSD。它适合海报拆层、产品场景拆图、AI 生图元素组装、白底转透明、颜色聚类拆层，以及“把图片拆成若干个 Photoshop 可移动图层”的工作流。
...
```text
使用 mmkc-creator-image2psd-skill 把这张图转成 PSD。
...
再用 mmkc-creator-image2psd-skill 把它们组装成 PSD。
```
